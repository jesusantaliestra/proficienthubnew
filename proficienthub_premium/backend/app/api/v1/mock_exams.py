...
from app.services.exceptions import InvalidExamTypeError, MockExamException

router = APIRouter(prefix="/mock-exams", tags=["Mock Exams"])

# Valid exam types from EXAM_TIME_CONFIG
...
async def list_mock_exams(
    ...
    status: Optional[str] = Query(None, description="Filter by status: not_started, in_progress, paused, completed"),
    current_user: User = Depends(get_current_user),
    session: AsyncSession = Depends(get_db_session)
) -> Dict[str, Any]:
    """
    List mock exams for the student for a specific exam type with pagination
    ...
    """
    service = MockExamService(session)
    all_exams = await service.get_student_mock_exams(current_user.id, exam_type)

    # Filter by status if provided
    if status:
        all_exams = [e for e in all_exams if e["status"] == status]

    total = len(all_exams)

    # Apply pagination
    paginated_exams = all_exams[skip:skip + limit]

    return {
        "exam_type": exam_type,
        "mock_exams": paginated_exams,
        "total": total,
        "skip": skip,
        "limit": limit,
...
async def create_mock_exam(
    ...
    Valid exam types: ielts_academic, ielts_general, cambridge_b2_first,
    cambridge_c1_advanced, toefl_ibt, pte_academic, oet_medicine
    """
    # Validate exam_type
    if request.exam_type not in VALID_EXAM_TYPES:
        logger.warning(
            "invalid_exam_type_requested",
            user_id=current_user.id,
            exam_type=request.exam_type,
            valid_types=VALID_EXAM_TYPES
        )
        raise HTTPException(
            status_code=400,
    ...
    logger.info(
        "mock_exam_created_api",
        user_id=current_user.id,
        mock_exam_id=result.get("id"),
        exam_number=result.get("exam_number")
    )
...
async def complete_section(
    ...
    logger.info(
        "section_completed_api",
        user_id=current_user.id,
        mock_exam_id=mock_exam_id,
        section_type=request.section_type,
        band_score=result.get("band_score"),
        all_complete=result.get("all_sections_complete", False)
    )
